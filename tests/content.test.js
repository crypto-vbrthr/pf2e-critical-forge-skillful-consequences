...
test("dev.1 contains twelve unique Physical Actions cards", () => {
  assert.equal(PHYSICAL_ACTION_CARDS.length, 12);
  assert.equal(new Set(PHYSICAL_ACTION_CARDS.map((card) => card.id)).size, 12);
  assert.equal(new Set(PHYSICAL_ACTION_CARDS.map((card) => card.fallbackTitle)).size, 12);
});

test("Grapple and Trip each have a complete 3 + 3 mini-deck", () => {
...
test("all current cards are Athletics skill-deck cards with exact action filters", () => {
  for (const card of PHYSICAL_ACTION_CARDS) {
    ...
    assert.equal(["grapple", "trip"].includes(card.filters.actionSlugs[0]), true, card.id);
...
test("the first physical block avoids strong consequences", () => {
  assert.equal(PHYSICAL_ACTION_CARDS.some((card) => card.impact === "strong"), false);
  assert.ok(PHYSICAL_ACTION_CARDS.filter((card) => card.impact === "narrative").length >= 2);
});
...
test("pack topology reserves future families while only Physical Actions is enabled", () => {
  ...
  assert.equal(packs[0].decks.skill.cards.length, 12);
